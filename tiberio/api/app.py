...
from tiberio.api.logging_setup import configure_logging
from tiberio.api.middleware import (
...
from tiberio.composition import Container, build_container
from tiberio.config.settings import Settings, get_settings
from tiberio.interfaces.alexa.directive_router import alexa_router
from tiberio.interfaces.http_auth import require_bearer_token
from tiberio.interfaces.oauth.router import oauth_router
...
def _validate_security_settings(settings: Settings) -> None:
    ...
    if not secret:
        raise RuntimeError(
            "TIBERIO_JWT_SECRET is not set. Refusing to start: an empty JWT "
            "secret makes every access token forgeable. Set TIBERIO_JWT_SECRET "
            "or TIBERIO_DEV_MODE=true for local development."
        )
    if len(secret) < _MIN_JWT_SECRET_LENGTH:
        raise RuntimeError(
            "TIBERIO_JWT_SECRET is too short for HS256: "
            f"need at least {_MIN_JWT_SECRET_LENGTH} characters."
        )
    if not settings.shared_secret.get_secret_value():
        log.warning(
            "TIBERIO_SHARED_SECRET is not set — HMAC request signing on "
...
def _validate_beacon_settings(settings: Settings) -> None:
    ...
    if not url:
        raise RuntimeError(
            "TIBERIO_BEACON_ENABLED is true but TIBERIO_PUBLIC_BASE_URL is not "
    ...
    if not url.startswith(allowed):
        raise RuntimeError(
            "TIBERIO_PUBLIC_BASE_URL must start with https:// "
            "(http:// is allowed only with TIBERIO_DEV_MODE=true): "
            f"got {url!r}."
        )
    ...
    if not settings.oauth_allowed_redirect_uris:
        if settings.dev_mode:
            ...
            log.error(
                ...
                "Set TIBERIO_OAUTH_ALLOWED_REDIRECT_URIS or TIBERIO_DEV_MODE=true."
            )

    _register_routes(app)

    log.info("tiberio server created")
    return app
...
def main() -> None:
    ...
    from tiberio.config.settings import _PROJECT_ROOT  # noqa: PLC0415
