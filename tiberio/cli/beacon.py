"""tiberio-beacon — CLI to publish the S3 endpoint beacon (KONZEPT §9).
...
app = typer.Typer(
    name="tiberio-beacon",
    help="Publish the tiberio S3 endpoint beacon.",
...
@app.callback()
def _callback() -> None:
    """Manage the tiberio S3 endpoint beacon."""
...
def publish(
    base_url: Annotated[
        ...
        typer.Option(
            "--base-url",
            "-u",
            help="Public base URL to announce (defaults to TIBERIO_PUBLIC_BASE_URL).",
            show_default=False,
        ),
    ] = None,
    bucket: Annotated[
    ...
    if not resolved_base_url:
        typer.echo(
            "Error: no base URL given (pass --base-url or set TIBERIO_PUBLIC_BASE_URL).",
            err=True,
        )
        raise typer.Exit(1)
