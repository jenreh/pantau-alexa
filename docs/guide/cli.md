...
tiberio ships three executables, all installed by `uv sync`:
...
PORT=9000 uv run tiberio

# Run in debug mode (auto-reload on code changes)
DEBUG=true uv run tiberio
```
...
All subcommands accept a `--db` option to specify the SQLite database path. If omitted, the path is read from the `USERS_DB_PATH` environment variable or the default (`tiberio_users.db`).
...
an interval (when `TIBERIO_BEACON_ENABLED=true`); use the CLI to publish on
...
3. Default: `tiberio_users.db` (in the current directory)

This makes it easy to point the CLI at the same database the server uses:

```bash
export USERS_DB_PATH=/var/lib/tiberio/users.db
uv run tiberio-users list
uv run tiberio-users add alice
```
...
echo 'USERS_DB_PATH=tiberio_users.db' >> .env
